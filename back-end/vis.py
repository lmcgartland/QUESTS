...
class Selfstats:
    ...
    def show_summary(self):
        ...
        # output_string += '%d keystrokes in %d key sequences,' % (self.summary.get('keystrokes', 0), self.summary.get('nr', 0))
        # output_string += '%d clicks (%d excluding scroll),' % (self.summary.get('clicks', 0), self.summary.get('noscroll_clicks', 0))
        # output_string += '%d mouse movements' % (self.summary.get('mousings', 0))
        outputDictionary = {}
        outputDictionary['keystrokes'] = self.summary.get('keystrokes', 0)
        outputDictionary["clicks"] = self.summary.get('clicks', 0)
        outputDictionary["noscroll_clicks"] = self.summary.get('noscroll_clicks', 0)
        ...
        if self.args['pactive']:
            ...
            for name, data in pdata:
                ...
                outputDictionary[name] = data['active_time']
            #print
